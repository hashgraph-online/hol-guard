...
@pytest.mark.parametrize("destination", [
    "https://attacker.invalid/audit", "http://hol.org/audit", "https://hol.org.attacker.invalid/audit",
    "https://user:password@hol.org/audit", "https://hol.org/audit#fragment",
    "https://staging.hol.org/audit", "http://127.0.0.1:3000/audit", "https://hol.org:444/audit",
    "//attacker.invalid/audit", "/audit",
])
def test_untrusted_audit_destination_never_receives_headers_or_network_access(
    ...
    with pytest.raises(ValueError):
        local_supply_chain._execute_cloud_workspace_audit_request(
            ...
            request_url=destination, method="POST", payload={"packages": []},
        )
...
def test_invalid_sync_authority_is_rejected_before_headers(sync_url: object) -> None:
    ...
    with pytest.raises(ValueError):
        build_cloud_workspace_audit_request(
            auth_context={"sync_url": sync_url}, request_url="https://hol.org/audit",
            method="GET", payload=None, build_headers=headers,
        )
    headers.assert_not_called()


@pytest.mark.parametrize("origin", ["https://hol.org", "https://staging.hol.org", "http://127.0.0.1:3000", "http://[::1]:3000"])
...
def test_trusted_same_origin_preserves_method_body_and_query(origin: str, method: str, payload) -> None:
    ...
    request = build_cloud_workspace_audit_request(
        auth_context=context, request_url=url, method=method, payload=payload, build_headers=headers,
    )
...
def test_explicit_staging_origin_is_preserved(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    request = build_cloud_workspace_audit_request(
        ...
        method="GET", payload=None, build_headers=Mock(return_value={}),
    )
