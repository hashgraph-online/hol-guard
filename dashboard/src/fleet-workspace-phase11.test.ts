...
function testSupportedAppSlugs(): void {
  ...
  assert(SUPPORTED_APP_SLUGS.length === expected.length, `Expected ${expected.length} slugs, got ${SUPPORTED_APP_SLUGS.length}`);
}
...
function testSupportedAppsBriefMentionsAllApps(): void {
  ...
  assert(brief.includes("cline"), "SUPPORTED_APPS_BRIEF should mention Cline");
