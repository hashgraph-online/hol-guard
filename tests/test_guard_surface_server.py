...
class TestGuardSurfaceServer:
    ...
    def test_guard_daemon_returns_not_found_for_unknown_client_attach_sessions(self, tmp_path) -> None:
        store = GuardStore(tmp_path / "guard-home")
        daemon = GuardDaemonServer(store, host="127.0.0.1", port=0)
        daemon.start()
        ...
        try:
            ...
            missing_attach_request = urllib.request.Request(
                f"http://127.0.0.1:{daemon.port}/v1/clients/attach",
                data=json.dumps(
                    {
                        "client_id": initialize_payload["client_id"],
            ...
            missing_error = None
            try:
                urllib.request.urlopen(missing_attach_request, timeout=5)
            except urllib.error.HTTPError as error:
                missing_error = error
        finally:
            daemon.stop()
        ...
        assert missing_error is not None
        assert missing_error.code == 404
        assert json.loads(missing_error.read().decode("utf-8")) == {
            "attached": False,
            "error": "not_found",
        }

    def test_guard_daemon_returns_not_found_for_unknown_operation_item_writes(self, tmp_path) -> None:
        store = GuardStore(tmp_path / "guard-home")
        daemon = GuardDaemonServer(store, host="127.0.0.1", port=0)
        daemon.start()
        ...
        try:
            ...
            missing_item_request = urllib.request.Request(
                f"http://127.0.0.1:{daemon.port}/v1/operations/missing/items",
                data=json.dumps(
                    {
                        "item_type": "approval_requested",
                        "payload": {"approval_requests": []},
            ...
            missing_error = None
            try:
                urllib.request.urlopen(missing_item_request, timeout=5)
            except urllib.error.HTTPError as error:
                missing_error = error
        finally:
            daemon.stop()
