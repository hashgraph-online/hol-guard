...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hol-guard-native-posttool-") as temporary:
        ...
        bad_config = _request(workspace, rule_digest, "safe\n", time.time_ns())
        bad_config["policy_snapshot"]["config_digest"] = "f" * 64
        bad_config_result = _run(runtime, bad_config)
        if bad_config_result.returncode == 0:
            decoded = _decode(bad_config_result)
            if decoded is not None and _decision(decoded) == "allow":
                raise SystemExit("config-mismatched policy snapshot was allowed")
        evidence.append({"case": "config-mismatch", "returncode": bad_config_result.returncode})
        ...
        oversized = _run(
            runtime,
            _request(workspace, rule_digest, "x" * (7 * 1024 * 1024), time.time_ns()),
        )
