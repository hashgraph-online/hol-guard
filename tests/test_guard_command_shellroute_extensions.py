...
SHELLROUTE_WRAPPER_REVIEW_COMMANDS: tuple[tuple[str, str], ...] = (
    ...
    # npm-style runners: command.package.node reviews the package, not the operation.
    ("npx shellroute reveal-key", _REVEAL_RULE),
    ("npx -y shellroute run US -- curl https://example.com", _RUN_RULE),
    ("npx --package shellroute shellroute proxy stop", _PROXY_RULE),
    ("bunx shellroute reveal-key", _REVEAL_RULE),
    ("pnpm shellroute proxy --country US", _PROXY_RULE),
    ("yarn shellroute run US -- curl https://example.com", _RUN_RULE),
    ("npm exec shellroute reveal-key", _REVEAL_RULE),
    ("pnpm exec shellroute proxy stop", _PROXY_RULE),
    ("pnpm dlx shellroute reveal-key", _REVEAL_RULE),
    ("yarn dlx shellroute run US -- curl https://example.com", _RUN_RULE),
...
SHELLROUTE_SAFE_COMMANDS: tuple[str, ...] = (
    ...
    "npx shellroute status",
    "npx -y shellroute countries",
    "bunx shellroute balance",
    "pnpm dlx shellroute status",
    "npm exec shellroute reveal-key --help",
