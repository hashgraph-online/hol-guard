...
def test_install_writes_fail_closed_pretool_hook(tmp_path: Path) -> None:
    ...
    guard_entries = [entry for entry in entries if "bounded_cli_hook_bridge" in str(entry.get("command", ""))]
    ...
    assert "bounded_cli_hook_bridge" in allowlist["approvals"][0]["command"]
...
def test_install_is_idempotent_for_runtime_hooks(tmp_path: Path) -> None:
    ...
    guard_entries = [
        ...
        if "bounded_cli_hook_bridge" in str(entry.get("command", ""))
    ]
    assert len(guard_entries) == 1
...
def test_hermes_bridge_maps_review_to_block() -> None:
    stdout, _stderr, code = hermes_bridge_response(
    ...
    assert code == 2
    native_stdout, _native_stderr, native_code = hermes_bridge_response(
