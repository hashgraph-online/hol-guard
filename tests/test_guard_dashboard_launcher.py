...
class TestOpenDashboardSuccess:
    ...
    def test_browser_opener_failure_is_reported_with_public_url(self, tmp_path: Path) -> None:
        mock_store = MagicMock()
        mock_config = MagicMock()
        mock_config.approval_surface_policy = "auto-open-once"
        mock_surface = MagicMock()
        ...
        with (
            ...
            def ensure_surface(**kwargs: object) -> dict[str, object]:
                ...
                browser_url = str(kwargs["browser_url"])
                opened = mock_opener(browser_url)
                return {
                    "opened": opened,
                    "reason": "opened" if opened else "open-failed",
                }
