"""Behavior-fixture regression for the declarative command.keibidrop source."""
...
_SOURCE_PATH = ROOT / "contributions/command-sources/command.keibidrop.json"
_FIXTURE_PATH = ROOT / "tests/fixtures/command-source-keibidrop.v1.json"
...
def test_keibidrop_source_is_present_and_declares_its_rules() -> None:
    source = json.loads(_SOURCE_PATH.read_text())
    assert source["schema"] == "guard.command-extension-source.v1"
    assert source["extension"]["extension_id"] == "command.keibidrop"
...
def test_keibidrop_portable_fixture_binds_canonical_sources() -> None:
    ...
    assert "command.keibidrop" in ids
...
def test_keibidrop_behavior_fixtures_pass_against_the_native_evaluator() -> None:
    """Runs the fixture cases, the wrapper and disabled-permission cases
    included, through the real compiled catalog via `guard-command-source test`."""
