...
def test_grok_generic_hook_resumes_after_approval(tmp_path: Path, monkeypatch) -> None:
    ...
    config = GuardConfig(
        guard_home=guard_home,
        workspace=tmp_path,
        approval_wait_timeout_seconds=4,
    )
    (guard_home / "config.toml").write_text("approval_wait_timeout_seconds = 4\n", encoding="utf-8")
    monkeypatch.setattr(
        commands_hook_generic,
        "schedule_guard_daemon_ensure",
        lambda *_args, **_kwargs: "http://127.0.0.1/pending",
    )
    monkeypatch.setattr(
        sys,
        "argv",
        ["hol-guard", "hook", "--guard-home", str(guard_home), "--harness", "grok"],
    )
    args = argparse.Namespace(
        harness="grok",
        json=False,
