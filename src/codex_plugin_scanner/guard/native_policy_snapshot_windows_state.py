...
def _windows_ensure_private_directory(path: Path) -> None:
    ...
    try:
        ...
        kernel32, handle, _information = api._windows_open_handle(path, directory=True)
        try:
            if created:
                with api._windows_private_descriptor(True) as (_advapi32, descriptor, dacl, _owner_sid):
