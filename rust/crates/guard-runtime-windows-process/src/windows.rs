...
use winapi::um::processthreadsapi::{
    ...
    InitializeProcThreadAttributeList, OpenProcess, UpdateProcThreadAttribute, PROCESS_INFORMATION,
};
#[cfg(test)]
...
pub use source_files::{SourceFile, SourceFileIdentity};
...
impl ManagedChild {
    ...
    pub fn terminate_with_timeout(&self, timeout: std::time::Duration) -> io::Result<()> {
        process_cleanup::terminate_and_wait(&self.process, Some(&self.job), timeout)
    }

    /// Observe an empty Job after an already observed child exit. Returns false
    /// when lingering descendants required forced termination within the budget.
    pub fn retire_after_exit_with_timeout(&self, timeout: std::time::Duration) -> io::Result<bool> {
        process_cleanup::retire_after_exit(&self.process, &self.job, timeout)
    }

    /// Release only this Job's close-kill flag after authenticated startup handoff.
    /// The serving supervisor must retain its own close-kill Job for descendants.
    pub fn allow_detached_lifetime(&self) -> io::Result<()> {
        process_lifecycle::set_job_kill_on_close(&self.job, false)
    }
}
...
pub fn spawn_managed_child(executable: &Path, args: &[&OsStr]) -> io::Result<ManagedChild> {
    spawn_managed_child_with_setup(executable, args, &mut spawn_setup::WindowsSpawnSetup)
}

/// Classify a managed process error whose bounded cleanup also failed.
pub fn managed_cleanup_failed(error: &io::Error) -> bool {
    process_cleanup::has_cleanup_failure(error)
}

fn spawn_managed_child_with_setup(
    executable: &Path,
    args: &[&OsStr],
    setup: &mut impl spawn_setup::SpawnSetup,
    ...
    let job = spawn_setup::contain_and_resume(&process, &thread, setup)?;
    drop(thread);
...
#[cfg(test)]
#[path = "spawn_setup_tests.rs"]
mod spawn_setup_tests;
