...
def _generated(context: HarnessContext, mode: str, monkeypatch: pytest.MonkeyPatch) -> tuple[str, ...]:
    ...
    return (
        "/bin/sh", "-c", proxy._DESKTOP_PROXY_LAUNCH_SCRIPT, "hol-guard-desktop-proxy",
        str(candidate), "TEAM123", str(bundle), json.dumps(config), str(core),
    )
...
@pytest.mark.security_critical
@pytest.mark.parametrize("option", ["--guard-home", "--home", "--workspace"])
def test_installed_readiness_rejects_consistent_path_redirects(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, option: str) -> None:
