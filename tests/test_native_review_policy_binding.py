...
def _review(worker, tmp_path: Path, *, payload: dict | None = None) -> dict:
    return worker.review_http_payload(
        payload=payload
        or {"hook_event_name": "PreToolUse", "tool_name": "Bash", "tool_input": {"command": "ollama push test"}},
        params={},
...
def _resolve(store, response: dict) -> str:
    ...
    assert store.resolve_harness_native_approval_request(
        ...
        resolved_at="2026-09-17T00:01:00+00:00",
        expected_harness="cursor",
    )
    return request_id


def test_bound_review_records_domain_and_reuses_without_receipt_persistence(tmp_path, monkeypatch) -> None:
    edge = _bound_edge()
    ...
    try:
        ...
        # Snapshot renewal changes generation, not the authenticated policy
        # domain. Ordinary local reuse is intentionally not a one-use claim.
