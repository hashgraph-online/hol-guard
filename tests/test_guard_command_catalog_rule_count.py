...
def test_command_extension_registry_is_deterministic_and_complete() -> None:
    ...
    assert BUILT_IN_COMMAND_EXTENSION_REGISTRY.for_action_class("PromptBranch prompt publication command") is not None
    assert sum(extension["rule_count"] for extension in payload["extensions"]) == 243
