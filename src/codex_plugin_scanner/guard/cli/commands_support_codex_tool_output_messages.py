...
def _codex_tool_output_request_summary(
    *,
    ...
    if local_secret_source is not None:
        return (
            f"{harness_label} tool `{tool_name}` read local secrets from {local_secret_source} "
            f"while running `{command_text}`."
        )
    if focused_pytest and merged_output_capture:
        return (
            f"{harness_label} tool `{tool_name}` ran focused pytest, merged stderr into stdout while running "
            f"`{command_text}`, and the captured output looked credential-like."
        )
    if focused_pytest:
        return (
            f"{harness_label} tool `{tool_name}` ran focused pytest and produced credential-looking output while "
            f"running `{command_text}`."
        )
    if merged_output_capture:
        return (
            f"{harness_label} tool `{tool_name}` merged stderr into stdout while running `{command_text}`, "
            "and the captured output looked credential-like."
        )
    return f"{harness_label} tool `{tool_name}` produced credential-looking output while running `{command_text}`."
...
def _codex_tool_output_runtime_summary(
    ...
    *,
    ...
    if local_secret_source is not None:
        return f"Local secrets from {local_secret_source} reached {harness_label} tool output."
    if focused_pytest and merged_output_capture:
        return (
    ...
    if merged_output_capture:
        return f"Combined stdout/stderr looked credential-like before it reached {harness_label}."
    return f"Requests a sensitive native tool action: credential-looking output reached {harness_label}."
...
def _codex_tool_output_runtime_reason(
    ...
    *,
    ...
    if local_secret_source is not None:
        return (
            f"Guard inspects supported {harness_label} tool output before {harness_label} uses it, "
            "so accidental secret reads can be "
    ...
    if merged_output_capture:
        return (
            ...
            f"output to {harness_label}. If you only need the exit status, rerun without `2>&1` or keep stderr out of "
            "the "
            "model-visible output."
        )
    return (
        f"Guard inspects supported {harness_label} tool output before {harness_label} uses it, "
        "so accidental secret reads can be stopped "
