    ...
    *,
    ...
    for row in rows:
        ...
        if row.get("resolution_action") != "allow":
            continue
        snapshot = row.get("continuation_snapshot")
        if harness == "codex" and isinstance(snapshot, Mapping) and snapshot.get("capability") == "suspended-response":
            # Browser continuation consumes its exact local-once authority at
            # the live-decision endpoint, never through reusable resolved rows.
            continue
