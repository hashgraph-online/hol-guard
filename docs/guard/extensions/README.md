...
| `command.sandbin` | Reviews sandbin commands that submit a new remote sandbox job or mint a new API key, while leaving read-only commands (languages, keys status, run --reconnect) unreviewed. | 2 | Built in |
