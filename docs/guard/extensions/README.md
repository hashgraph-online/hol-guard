...
| `command.codesage` | Python reference rules for CodeSage hook installation and MCP registration. Production native hooks do not consume these rules. Enabling the extension does not make doctor, status, or search automatic. | 3 | External opt-in |
