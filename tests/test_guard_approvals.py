...
class TestGuardApprovals:
    ...
    def test_guard_daemon_approve_route_accepts_same_origin_local_browser_without_header_token(self, tmp_path):
        store = GuardStore(tmp_path / "guard-home")
        store.add_approval_request(
            GuardApprovalRequest(
                request_id="req-same-origin-http",
                harness="codex",
                artifact_id="codex:project:workspace_skill",
                artifact_name="workspace_skill",
                artifact_hash="hash-same-origin",
                policy_action="require-reapproval",
                recommended_scope="artifact",
                changed_fields=("args",),
                source_scope="project",
                config_path=str(tmp_path / "workspace" / ".codex" / "config.toml"),
                review_command="hol-guard approvals approve req-same-origin-http",
                approval_url="http://127.0.0.1/pending",
            ),
            "2026-04-11T00:00:00+00:00",
        )
        ...
        try:
            request = urllib.request.Request(
                f"http://127.0.0.1:{daemon.port}/v1/requests/req-same-origin-http/approve",
                data=json.dumps({"scope": "global", "reason": "approved in review"}).encode("utf-8"),
                headers={
        ...
        assert status == 200
        assert payload["resolved"] is True
        assert payload["resolved_request"]["resolution_action"] == "allow"
        assert payload["resolved_request"]["resolution_scope"] == "global"

    def test_guard_daemon_approve_route_rejects_wrong_port_without_header_token(self, tmp_path):
        store = GuardStore(tmp_path / "guard-home")
        store.add_approval_request(
            GuardApprovalRequest(
                request_id="req-wrong-origin-http",
                harness="codex",
                artifact_id="codex:project:workspace_skill",
                artifact_name="workspace_skill",
                artifact_hash="hash-wrong-origin",
                policy_action="require-reapproval",
                recommended_scope="artifact",
                changed_fields=("args",),
                source_scope="project",
                config_path=str(tmp_path / "workspace" / ".codex" / "config.toml"),
                review_command="hol-guard approvals approve req-wrong-origin-http",
                approval_url="http://127.0.0.1/pending",
            ),
            "2026-04-11T00:00:00+00:00",
        )
        ...
        try:
            request = urllib.request.Request(
                f"http://127.0.0.1:{daemon.port}/v1/requests/req-wrong-origin-http/approve",
            ...
            try:
                ...
                raise AssertionError("expected HTTPError for wrong same-origin port")
        finally:
            daemon.stop()
