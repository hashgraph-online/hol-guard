...
class _CloudReviewHandler(Protocol):
    ...
    def _codex_resume_after_exact_review(self, **kwargs: object) -> dict[str, object] | None: ...
...
def _handle_remote_exact(handler: _CloudReviewHandler, payload: dict[str, object]) -> None:
    status, response = build_headless_exact_cloud_review_response(
        ...
        resume_codex=handler._codex_resume_after_exact_review,
...
def build_headless_exact_cloud_review_response(
    *,
    ...
    signed_decision = decode_mapping(payload.get("remoteApproval"))
    ...
    if adapter.harness == "codex":
        ...
        if codex_resume is not None:
            response["codexResume"] = codex_resume
    else:
        try:
        ...
        if harness_resume is not None:
            detail = harness_resume.get("harnessResume")
            response["harnessResume"] = detail if isinstance(detail, dict) else harness_resume
            for key in ("continuationCompletedAt", "continuationReason", "continuationStatus"):
                if key in harness_resume:
                    response[key] = harness_resume[key]
    response["deliveryStatus"] = "incomplete" if post_commit_errors else "completed"
    if post_commit_errors:
        response["postCommitErrors"] = post_commit_errors
    return 200, response
