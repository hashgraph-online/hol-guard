...
_COMMON_FLAGS = frozenset({"--help", "--verbose"})
...
_CONTAINER_RESOURCE_REMOVAL = _any(
    *(
        matcher
        for path in (
            ("container", "rm"),
        ...
        for matcher in _container(*path).matchers
    )
)
...
_CONTAINER_EXECUTION = _any(
    *_container("run").matchers,
    *_container("container", "run").matchers,
    *_container("exec").matchers,
    *_container("container", "exec").matchers,
)
_KUBE_DELETE = _kube("delete")
_KUBE_DRAIN = _any(
    *_kube("drain").matchers,
    executable_matcher(
        "oc",
        "adm",
        "drain",
...
_OC_DRY_RUN_MUTATION = _any(
    *(
        ...
        for path in (("apply",), ("patch",), ("scale",))
    )
)
_OC_MUTATION = _any(
    *_OC_DRY_RUN_MUTATION.matchers,
    *(
        executable_matcher(
            "oc",
            *path,
...
_HELM_UNINSTALL = _any(
    executable_matcher(
        "helm",
        "uninstall",
        global_options_with_values=_HELM_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    )
)
_IAC_DESTROY = _any(
    executable_matcher(
        "terraform",
        "destroy",
        global_options_with_values=_TERRAFORM_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    ),
    executable_matcher(
        "tofu",
    ...
    executable_matcher(
        "pulumi",
        "destroy",
        global_options_with_values=_PULUMI_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    ),
    executable_matcher(
        "cdk",
        "destroy",
        global_options_with_values=_CDK_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    ),
    executable_matcher(
        "sam",
        "delete",
        global_options_with_values=_SAM_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    ),
    executable_matcher(
        "serverless",
        "remove",
        global_options_with_values=_SERVERLESS_GLOBAL_OPTIONS,
        global_flags=_COMMON_FLAGS,
    ),
    executable_matcher(
        "sls",
...
DOMAIN_COMMAND_RULES = (
    _rule(
        ...
        description="Identifies broad cleanup of unused Docker, Podman, or nerdctl state.",
    ...
    _rule(
        ...
        safe_variants=(
            safe_flag_variant(_CONTAINER_FORCE_REMOVE, variant_id="help", title="Command help", flag="--help"),
        ),
    ),
    _rule(
        rule_id="command.container-runtime.resource-removal",
        title="Container runtime resource removal",
        description="Identifies container, image, volume, network, and builder cleanup across supported runtimes.",
        matcher=_CONTAINER_RESOURCE_REMOVAL,
        action_class="docker-sensitive command",
        risk_classes=("destructive_shell",),
        safer_alternative="List the exact runtime resources before deleting or pruning them.",
        safe_variants=(
            safe_flag_variant(_CONTAINER_RESOURCE_REMOVAL, variant_id="help", title="Command help", flag="--help"),
        ),
    ),
    _rule(
        ...
        severity="critical",
        safe_variants=(
            safe_flag_variant(_CONTAINER_PRIVILEGED, variant_id="help", title="Command help", flag="--help"),
        ),
    ),
    _rule(
        rule_id="command.container-runtime.execution",
        title="Container runtime execution",
        description="Identifies ordinary container launch or execution across Docker, Podman, and nerdctl.",
        matcher=_CONTAINER_EXECUTION,
        action_class="docker-sensitive command",
        risk_classes=("network_egress", "execution"),
        safer_alternative="Use a pinned image, minimal mounts and capabilities, and only the required environment inputs.",
        safe_variants=(
            safe_flag_variant(_CONTAINER_EXECUTION, variant_id="help", title="Command help", flag="--help"),
        ),
    ),
    _rule(
        ...
        safe_variants=(
            ...
            safe_option_variant(
                _KUBE_DELETE,
                variant_id="dry-run",
                title="Kubernetes deletion preview",
                option="--dry-run",
    ...
    _rule(
        ...
        description="Identifies OpenShift apply, patch, scale, rollout, project, cordon, and uncordon operations.",
        ...
        safe_variants=(
            safe_flag_variant(_OC_MUTATION, variant_id="help", title="Command help", flag="--help"),
            safe_option_variant(
                _OC_DRY_RUN_MUTATION,
                variant_id="dry-run",
    ...
    _rule(
        ...
        severity="critical",
        safe_variants=(
            safe_flag_variant(_IAC_DESTROY, variant_id="help", title="Command help", flag="--help"),
            CommandSafeVariant(
    ...
    _rule(
        ...
        action_class="infrastructure destructive command",
        risk_classes=("destructive_shell", "network_egress", "execution"),
        safer_alternative="Inspect the generated change set, diff, or package before applying production changes.",
        safe_variants=(
            safe_flag_variant(_IAC_PRODUCTION_CHANGE, variant_id="help", title="Command help", flag="--help"),
        ),
    ),
)
...
DOMAIN_COMMAND_EXTENSION_SPECS = (
    ...
    CommandExtensionSpec(
        ...
        description=(
            "Reviews teardown and high-impact deployment through Terraform, OpenTofu, Pulumi, AWS CDK/SAM, "
