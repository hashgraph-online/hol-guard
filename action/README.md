...
- Sync the install metadata files (`scanner-version.txt` and `pypi-attestations-version.txt`) with the action bundle so both action repositories always install the same reviewed scanner release.
