    ...
    *,
    ...
    with ExitStack() as lifetime:
                    ...
                    raise FixtureFailureError(
                        {
                            **failure_evidence(error),
                            "workspace_observation": workspace_fixture.startup_failure(),
                        }
                    ) from error
                raise
        ...
        with fault_context as fault:
            _serve_session(session, fault, workspace_fixture)
    _emit({"closed": True})
    return 0


def _serve_session(session: Any, fault: Any, workspace_fixture: Any = None) -> None:
    ...
    try:
        ...
        while True:
            ...
            elif isinstance(operation, str) and operation.startswith("workspace_") and workspace_fixture is not None:
                _emit(workspace_fixture.dispatch(operation, request))
