...
def test_write_guard_daemon_state_hardens_permissions_on_open_descriptor(tmp_path, monkeypatch):
    ...
    hardened_paths = (
        daemon_discovery_key_path(guard_home),
        daemon_manager_module._auth_token_path(guard_home),
        daemon_manager_module._state_path(guard_home),
    )
    hardened_files = {(path.stat().st_dev, path.stat().st_ino) for path in hardened_paths}
    written_modes = [mode for identity, mode in fchmod_calls if identity in hardened_files]
    assert len(written_modes) == 3
    assert all(mode == 0o600 for mode in written_modes)
