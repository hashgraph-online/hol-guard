...
from ..native_policy_snapshot import publish_native_policy_snapshot
...
def _workspace_identity(workspace: Path | None) -> str | None:
...
class HookWorker:
    ...
    def __init__(
        ...
        *,
        ...
        self._workspace_identity = _workspace_identity(workspace)
        ...
        self._observe_mode_value = False
        self._policy_snapshot_generation: int | None = None
        self._policy_snapshot_digest: str | None = None
        self.refresh_policy_snapshot(workspace=workspace, policy_epoch=policy_epoch)

    def refresh_policy_snapshot(self, *, workspace: Path | None, policy_epoch: int | None = None) -> None:
        """Refresh immutable native policy at a bounded control-plane epoch."""

        observe_mode = False
        generation: int | None = None
        snapshot_digest: str | None = None
        try:
            config = load_guard_config(self.guard_home, workspace=workspace)
            observe_mode = config.mode == "observe"
            installed = publish_native_policy_snapshot(
                self.store,
                workspace=workspace,
                config=config,
            )
            ...
            # Snapshot publication is a control-plane concern. Native requests
            # remain fail-safe if the resident cannot accept a fresh snapshot.
            observe_mode = False
            generation = None
            snapshot_digest = None
        with self._policy_lock:
            self._observe_mode_value = observe_mode
            self._policy_snapshot_generation = generation
            self._policy_snapshot_digest = snapshot_digest
            if policy_epoch is not None:
                self._policy_epoch = max(self._policy_epoch, policy_epoch)

    def ensure_policy_snapshot(self, *, workspace: Path | None, policy_epoch: int) -> None:
        """Select the bound workspace policy and refresh only when its epoch advances."""

        identity = _workspace_identity(workspace)
        if self._workspace_identity is not None and identity != self._workspace_identity:
            raise RuntimeError("native hook worker workspace binding mismatch")
        with self._policy_lock:
            current_epoch = self._policy_epoch
    ...
    def review_http_payload(
        ...
        *,
        ...
        self.ensure_policy_snapshot(workspace=workspace, policy_epoch=policy_epoch)
        ...
        with self._policy_lock:
            observe_mode = self._observe_mode_value
            policy_snapshot_digest = self._policy_snapshot_digest
        try:
