...
@pytest.mark.parametrize("expire_on_read", [1, 2, 3, 4])
def test_native_completion_keeps_reader_at_each_original_wait_check(tmp_path, monkeypatch, expire_on_read):
    monkeypatch.setenv("HOL_GUARD_NATIVE", "auto")
    store, workspace, edge, hook, row = _fixture(tmp_path)
    _resolve(store, row)
    worker = _Worker(store, edge)
    original_operation = store.get_guard_operation_for_approval_request
    started = (datetime.now(timezone.utc) - timedelta(seconds=5)).isoformat()

    def legacy_operation(request_id):
        operation = original_operation(request_id)
        operation["created_at"] = operation["updated_at"] = started
        operation["metadata"].pop("codex_browser_wait_deadline_at", None)
        return operation
    ...
    def read(path):
        if path.parent != store.guard_home:
            return {}
        reads.append(path)
        return {"approval_wait_timeout_seconds": 0 if len(reads) >= expire_on_read else 120}

    worker.config_reader = read
    monkeypatch.setattr(store, "get_guard_operation_for_approval_request", legacy_operation)
    result = _complete(store, workspace, edge, hook, row, worker=worker)
    assert result["completed"] is False
    # Even a failed completion keeps the existing final original-wait check.
    assert len(reads) == (4 if expire_on_read == 3 else expire_on_read)
    assert worker.calls == (0 if expire_on_read == 1 else 1)
