...
_WINDOWS_SOURCE_CAPABILITY = "post-tool-source-read-windows-handles-v1"


def installed_windows_source_reference_supported(runtime: Path) -> bool:
    """Select an oracle from the exact ready installed artifact's capability.

    The historical generic source feature also appeared on the Windows build
    that refused source references. It cannot establish Windows read support.
    This declaration only selects the expected contract; content and digest
    witnesses still have to pass before any sample qualifies.
    """
    status = native_runtime_status()
    if (
        status.mode != "auto"
        or status.available is not True
        or status.compatible is not True
