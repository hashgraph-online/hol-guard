    ...
    *,
    ...
    try:
        ...
        payload = validate_network_status(
            load_running_guard_surface_daemon_client(guard_home).network_status()
        )
    ...
    if reason_code is None and legacy_domain_action is not None:
        authoritative_policy = build_network_status(
            profiles=(), legacy_domain_action=legacy_domain_action
        ).get("legacy_domain_policy")
