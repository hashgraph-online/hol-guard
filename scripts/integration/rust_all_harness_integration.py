...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hol-guard-rust-harness-") as temporary:
            ...
            if not flush_native_route_metrics_report_for_tests(guard_home=guard_home):
                raise RuntimeError("guard-home native route report did not flush")
            ...
            if _contains_request_content(report_text) or str(root) in report_text:
                raise RuntimeError("guard-home native route report contained request-derived content")
