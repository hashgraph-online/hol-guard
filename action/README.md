...
Every release of this action is published with an immutable version tag (`v1.x.y`) plus the floating `v1` major tag. The primary usage example in the published action repository is rewritten to that release's immutable tag at publication time; the examples below keep illustrative pins. Pin the release exactly, or resolve it to a full commit SHA for the strictest form:

```yaml
# Recommended: full commit SHA
...
# Acceptable: immutable release tag (never re-pointed after publication)
...
| `online` | Permit network access: live probing for `verify` mode and env-configured external analyzer credentials. When `false`, `MCP_SCANNER_API_KEY` and `MCP_SCANNER_LLM_API_KEY` are removed from the scan environment so they cannot enable external analyzers | `false` |
...
- Rewrite the primary `uses:` example in the published README from the floating `@v1` form to the immutable `@v1.x.y` release tag when a bundle change is committed, so published examples never teach consumers to depend on a mutable ref; reruns without source changes are detected in canonical form and publish nothing.
