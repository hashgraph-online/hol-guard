...
def test_receipt_sync_atomically_accepts_managed_delivery_and_emits_exact_v2_ack(
    ...
    registry = guard_runner_module.BUILT_IN_COMMAND_EXTENSION_REGISTRY
    store._bootstrap_extension_control_authority(registry.catalog_digest, key=None)
    wire_catalog = guard_runner_module.build_builtin_extension_catalog_wire(
        guard_version="test",
        generated_at="2026-08-25T12:00:00Z",
    )
    wire_catalog_digest = wire_catalog["catalogDigest"]
    device_id, _device_name = guard_runner_module._guard_device_metadata(store)
    runtime_summary = {
        "runtime_session_id": "runtime-managed-controls",
        "runtime_device_id": device_id,
        "extensionCatalogDigest": wire_catalog_digest,
        "extensionAuthorityRevision": 7,
    }
    store.set_sync_payload("runtime_session_summary", runtime_summary, "2026-08-25T12:00:00Z")
    ...
    delivery = {
        ...
        "policyRevision": 7,
        "extensionAuthorityRevision": 7,
        "catalogDigest": wire_catalog_digest,
    ...
    response_payload = {
        ...
        "managedControlsCapabilities": sorted(guard_runner_module.MANAGED_CONTROLS_RUNTIME_CAPABILITIES),
    }
    ...
    context = guard_runner_module._receipt_sync_context(
        store,
        local_guard_online_at="2026-08-25T12:00:02Z",
    )
    assert context["policyBundleAcknowledgementV2"] == acknowledgement
            ...
            return json.dumps(
                {
                    ...
                    "receiptsStored": 0,
                    "policyBundle": bundle,
                    "managedControlsCapabilities": sorted(
                        guard_runner_module.MANAGED_CONTROLS_RUNTIME_CAPABILITIES
                    ),
                }
            ).encode("utf-8")
