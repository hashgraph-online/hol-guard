...
If you want to consume the action outputs inside the same workflow, `actions/github-script@v7` is a good fit for follow-up issue, PR, or summary automation.

## Release Management

- This is the primary published repository for the action.
- `hashgraph-online/ai-plugin-scanner` keeps the bundled action source in `action/`, and `.github/workflows/publish-action-repo.yml` is the source-repo workflow that prepares this published bundle.
