...
def test_pi_post_tool_output_labels_kubernetes_secret_source(tmp_path: Path) -> None:
    ...
    artifact = _codex_post_tool_output_artifact(
        ...
        payload={
            ...
            "stdout": "-----BEGIN\x20RSA PRIVATE KEY-----\nMIIE" + ("A" * 64) + "\n-----END RSA PRIVATE KEY-----\n",
        },
...
def test_read_only_command_batches_stay_read_only_for_post_tool_skip(tmp_path: Path) -> None:
    payload: dict[str, object] = {"tool_input": {"commands": ["rg foo src", "grep bar src/app.py"]}}
...
def test_quoted_kubectl_heredoc_sample_is_not_mislabeled() -> None:
    command = (
        "printf '%s\n' 'kubectl exec registry-frontend -- python - <<\"PY\"'\n"
        'printf "%s\n" \'import os; print(os.environ["GUARD_GITHUB_APP_PRIVATE_KEY"])\'\n'
        "printf '%s\n' 'PY'"
    )
