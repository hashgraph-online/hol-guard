...
def _derive_shell_semantics(envelope: Mapping[str, object], *, actor_label: str) -> ActionExplanationSemantics:
    ...
    if not effective:
        ...
        unknown = _unknown_semantics("shell_command", actor_label=actor_label, reason=reason)
        return ActionExplanationSemantics(
            **{**unknown.__dict__, "canonical_command": command, "canonical_identity": command.security_identity, "catalog_digest": BUILT_IN_COMMAND_EXTENSION_REGISTRY.catalog_digest, "confidence": "limited", "uncertainty_reasons": uncertainty or (reason,)}
        )
    ...
    if len(effective) == 1:
        ...
        impact = "The command combines multiple actions that can affect files, credentials, systems, or external services."
        alternatives = tuple(
            dict.fromkeys(
                alternative
        ...
        consequences = tuple(
            ExplanationConsequence(item.rule.description, item.rule.severity) for item in effective[:16]
        )
...
def _unknown_semantics(action_type: str, *, actor_label: str, reason: str) -> ActionExplanationSemantics:
    ...
    return ActionExplanationSemantics(
        ...
        summary=f"{actor_label} wants to perform an action. Guard could not confirm the exact intent from retained facts.",
        impact=impact,
        recommendation=recommendation,
        confidence="limited",
...
def _shell_targets(command: CanonicalCommand, observations: tuple[object, ...]) -> tuple[ExplanationTarget, ...]:
    indexes: list[int] = []
...
def _kind_for_rule(extension_id: str, rule: CommandSafetyRule) -> str:
    ...
    if extension_id == "command.git":
        if any(token in rule_id for token in ("reset", "rebase", "history", "force-push", "branch-delete")):
            return "git_history_rewrite"
        if any(token in rule_id for token in ("push", "remote", "fetch")):
...
def _typed_target(envelope: Mapping[str, object], kind: str) -> ExplanationTarget:
    ...
    if kind.startswith("network_"):
        ...
        return ExplanationTarget("network_host", f"the service {hosts[0]}" if hosts else "an external service")
    if kind == "mcp_tool":
        server = _text(envelope.get("mcp_server"))
        tool = _text(envelope.get("mcp_tool"))
        return ExplanationTarget("connected_tool", " / ".join(value for value in (server, tool) if value) or "a connected tool")
