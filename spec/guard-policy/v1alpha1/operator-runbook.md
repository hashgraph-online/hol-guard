...
This alpha release is promoted only by merging the Guard change into `release/3.0` and verifying its published alpha artifact against that release branch. It must not merge directly into `main`, publish a stable release, or enable canonical enforcement by default. Those actions belong to the later consolidated V3 release.
