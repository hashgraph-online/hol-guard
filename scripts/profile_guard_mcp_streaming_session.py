#!/usr/bin/env python3
"""Measure the actual MCP serve route with explicit streaming candidate F.

This separately named worker preserves the frozen E worker's client, child,
deadlines, quantiles and resource rules. Product default B remains unchanged.
...
def _worker(config_path: Path) -> int:
    ...
    native_pilot = None
    streaming_pilot = None
    loaded_adapter_sha256 = None
    if spec.get("streaming_preparation_pilot"):
        import guard_mcp_streaming_preparation_pilot as streaming_adapter
        from guard_mcp_streaming_preparation_pilot import OwnedPreparationPilot
        from guard_mcp_streaming_preparation_pilot import install_adapter as install_streaming_adapter

        loaded_adapter_sha256 = hashlib.sha256(Path(streaming_adapter.__file__).read_bytes()).hexdigest()
        streaming_pilot = OwnedPreparationPilot()
        install_streaming_adapter(runtime, streaming_pilot)
        if spec["profile"]:
            phases.wrap(streaming_pilot, "own_request", "owned_message_snapshot")
            phases.wrap(streaming_adapter, "_exact_binding", "owned_input_binding")
            phases.wrap(streaming_adapter, "_binding_matches", "owned_input_binding")
        ...
        json.dumps(
            {
                ...
                "streaming_preparation_pilot": streaming_pilot.evidence() if streaming_pilot is not None else None,
                "loaded_adapter_sha256": loaded_adapter_sha256,
...
def run_case(
    *,
    ...
    streaming_preparation_pilot: bool = False,
) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="guard-mcp-profile-") as directory:
        ...
        spec = {
            ...
            "streaming_preparation_pilot": streaming_preparation_pilot,
        ...
        try:
            ...
            return {
                ...
                "streaming_preparation_pilot": worker["streaming_preparation_pilot"],
            ...
            raise BenchmarkCaseError(
                {
                    ...
                    "streaming_preparation_pilot": failed_worker.get("streaming_preparation_pilot"),
...
def run_matrix(*, samples: int, output: Path, lock_file: Path | None = None, resume: bool = False) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "schema": "hol-guard-mcp-streaming-stdio-rebaseline.v1",
        "qualification": False,
