    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if (
                ...
                or key in FORBIDDEN
                or key.lower().endswith(("token", "secret", "password", "privatekey", "command", "script"))
            ):
        ...
        for item in value:
            reject_authority(item, depth + 1)
    elif isinstance(value, str) and len(value.encode()) > 16384:
...
def sign_config(value: dict[str, object], private_key: rsa.RSAPrivateKey) -> dict[str, object]:
    ...
    sig = private_key.sign(
        canonical(unsigned), padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=32), hashes.SHA256()
    )
...
def verify_config(
    ...
    *,
    ...
    clock = now or utcnow()
    if not issued <= clock and (issued - clock).total_seconds() > 300:
        raise ContractError("configuration_not_yet_valid")
    if clock < not_before or clock >= expires or expires <= issued or (expires - issued).total_seconds() > 86400:
        raise ContractError("configuration_expired")
    policy = validate_policy(root["policy"])
    expected = policy_hash(policy)
    if text(root["policyHash"], HEX64) != expected:
    ...
    if rollback["authorized"]:
        ...
        if not isinstance(rollback["reason"], str) or not rollback["reason"] or len(rollback["reason"]) > 512:
            raise ContractError("configuration_rollback_invalid")
...
def validate_ack(value: object, workspace: str, device: str, generation: str) -> dict[str, object]:
    ...
    if (root["status"] == "applied" and root["reasonCode"] is not None) or (
        root["status"] != "applied" and not isinstance(root["reasonCode"], str)
    ):
...
def validate_health(value: object, workspace: str, device: str, generation: str) -> dict[str, object]:
    ...
    if root["appliedRevision"] is None:
        ...
        text(root["appliedPolicyHash"], HEX64)
...
def sign_proof(
    private_key: ec.EllipticCurvePrivateKey, method: str, path: str, body: bytes, sequence: int, observed_at: str
) -> str:
    return base64.b64encode(
        private_key.sign(proof_message(method, path, body, sequence, observed_at), ec.ECDSA(hashes.SHA256()))
    ).decode()
