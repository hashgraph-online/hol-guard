...
def _run_phase(arguments: tuple[str, ...], root: Path, prior: dict | None):
    if arguments[-1] != "baseline_rollback" or sys.platform not in {"linux", "win32"}:
        return _run(arguments, root)
    expected_file = Path(arguments[arguments.index("--expected") + 1])
    if json.loads(expected_file.read_text())["build_sha"] != AUDITED_BASELINE_SHA:
        return _run(arguments, root)
    # The preceding candidate generation must have already supplied the same
    # positive/authenticated retirement proof required for every replacement.
    if prior is None or prior.get("phase") != "candidate_reinstall" or prior.get("passed") is not True:
        raise RuntimeError("qualification_transition_prior_cleanup_unverified")
    request = {
        "nonce": secrets.token_hex(32),
        "arguments": arguments,
        "argv_sha256": digest_json(arguments),
        "code_sha256": code_digest(),
        "installation": installation_pin(Path(arguments[0])),
        "prior_retirement_sha256": digest_json(prior),
    }
    destination = root / "quiescence-request.json"
    write_private(destination, request)
    result = _run(
        (
            str(Path(sys.executable).absolute()),
            "-I",
            str(Path(__file__).with_name("installed_transition_quiescence.py")),
            "--request",
            str(destination),
        ),
        root,
        # Worker allowance stays 180 seconds. Additional time belongs only to
        # bounded drain/cleanup and certificate hashing in the supervisor.
        timeout_seconds=195,
    )
    try:
        report = json.loads(result.stdout)
        proof = report.get("quiescence", {})
        if proof.get("verified") is True and (
            proof.get("request_sha256") != digest_json(request)
            or proof.get("nonce") != request["nonce"]
            or proof.get("argv_sha256") != request["argv_sha256"]
            or proof.get("code_sha256") != request["code_sha256"]
            or proof.get("installation_sha256") != digest_json(request["installation"])
            or not certificate_valid(report, prior_retirement_sha256=digest_json(prior))
        ):
            raise RuntimeError("qualification_transition_owner_binding_invalid")
    except (ValueError, AttributeError, TypeError):
        # worker_evidence retains this unparseable/incomplete result as failed.
        pass
    return result
...
def worker_evidence(
    ...
    phase: str,
    *,
    prior_receipts: int | None = None,
    prior_retirement_sha256: str | None = None,
) -> dict:
    try:
    ...
    verified_rejection = (
        ...
        and (
            (
                report.get("retirement_verification", {}).get("verified") is True
            ...
            or (
                prior_retirement_sha256 is not None
                and certificate_valid(report, prior_retirement_sha256=prior_retirement_sha256)
            )
        )
    ...
    try:
        ...
        for collection, plan, completed in ((compatible, _COMPATIBLE_PHASES, 3), (report, _PHASES, 4)):
            ...
            for row, (phase, arm) in zip(rows, plan, strict=True):
                ...
                checked = worker_evidence(
                    result,
                    contracts[arm],
                    phase,
                    prior_receipts=prior_receipts,
                    prior_retirement_sha256=digest_json(prior_row) if prior_row is not None else None,
                )
        ...
        with tempfile.TemporaryDirectory(prefix="hol-guard-artifact-transition-") as temporary:
            ...
            for phase, arm in plan:
                ...
                result = _run_phase(
                    ...
                    root,
                    report["phases"][-1] if report["phases"] else None,
                )
                observed = worker_evidence(
                    result,
                    contracts[arm],
                    phase,
                    prior_receipts=prior_receipts,
                    prior_retirement_sha256=digest_json(report["phases"][-1]) if report["phases"] else None,
                )
                if phase == "baseline_rollback":
                    try:
